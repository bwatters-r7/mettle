...
int loader_alloc(loader_t * loader, size_t sz)
{
	...
	loader->length = sz;
	loader->next = loader->base =
		mmap(NULL, sz, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_ANONYMOUS | MAP_PRIVATE, -1, 0);
...
int load_elf_blob(loader_t * loader, blob_t * blob_in, blob_t * blob_out)
{
	...
	if ((size_t)(loader->next) & (page_size - 1))
		crash();
	...
	for (i = 0; i < ehdr->e_phnum; i++, phdr++) {
		...
		if (!blob_out->blob) {
			...
			printf("Lost %zu bytes due to page alignment :/\n", diff);
		}
