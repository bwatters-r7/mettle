...
void trap_handler(int sig, siginfo_t * info, void *_ctx)
{
	...
	printf("trap_no: %08x, error_code: %08x, oldmask: %08x\n", mctx->trap_no, mctx->error_code,
		mctx->oldmask);
	printf("r0: %08x, r1: %08x, r2: %08x, r3: %08x\n", mctx->arm_r0, mctx->arm_r1, mctx->arm_r2,
		mctx->arm_r3);
	printf("r4: %08x, r5: %08x, r6: %08x, r7: %08x\n", mctx->arm_r4, mctx->arm_r5, mctx->arm_r6,
		mctx->arm_r7);
	printf("r8: %08x, r9: %08x, 10: %08x, fp: %08x\n", mctx->arm_r8, mctx->arm_r9, mctx->arm_r10,
		mctx->arm_fp);
	printf("ip: %08x, sp: %08x, lr: %08x, pc: %08x\n", mctx->arm_ip, mctx->arm_sp, mctx->arm_lr,
		mctx->arm_pc);
	...
	printf("%p %p %p\n", platform_arg(mctx, 0), platform_arg(mctx, 1), platform_arg(mctx, 2)
		);
	fflush(stdout);
	...
	if (!in_dlopen) {
		printf
			("shortcutting execution since not in dlopen / dlopenbuf cases we're interested in\n");
	...
	case OPEN_OFFSET:
		ret = my_open(&emulated,
			...
			(int)platform_arg(mctx, 1), (mode_t) platform_arg(mctx, 2)
			);
		break;
	case CLOSE_OFFSET:
		...
		break;
	case FSTAT_OFFSET:
		ret = my_fstat(&emulated, (int)platform_arg(mctx, 0), (struct stat *)platform_arg(mctx, 1)
			);
		break;
	case MMAP_OFFSET:
		ret = my_mmap(&emulated, platform_arg(mctx, 0),	// addr
		...
		break;
	case READ_OFFSET:
		ret = my_read(&emulated, (int)platform_arg(mctx, 0),	// fd
	...
	case PREAD_OFFSET:
		ret = my_pread(&emulated,
			...
			platform_arg(mctx, 1), (size_t) platform_arg(mctx, 2), (off_t) platform_arg(mctx, 3)
			);
		break;
...
int __futex_wait(volatile void *ftx, int val, const struct timespec *timeout)
{
	...
	if (r == EINTR || r == EINVAL || r == ETIMEDOUT)
		return r;
	return 0;
}
...
int main(int argc, char **argv, char **envp)
{
	...
	printf("loader_info remaining space = %d\n",
		loader_info.length - ((size_t) loader_info.next - (size_t) loader_info.base)
		);
